...
def init(context):
    ...
    context.contracts = rqdatac.options.get_contracts("CU", "C", maturity="2107", trading_date=date.today())
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        mod_config = config["mod"]["vnpy"]
        ctp_settings: dict = mod_config["gateways"]["FUTURE"]["settings"]
        _, ctp_settings["用户名"], ctp_settings["密码"] = sys.argv
        if time(15, 30) <= datetime.now().time() < time(21):
            # simnow 24 小时服务器
            mod_config["full_day_mode"] = True
